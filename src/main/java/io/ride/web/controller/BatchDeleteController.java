...
public class BatchDeleteController {
    ...
    @DeleteMapping(value = "/rent/{arg}")
    public Result deleteRentBatch(@PathVariable("arg") String arg, HttpSession session) {
        LOGGER.info("待删除租用信息: {}", Arrays.toString(ParamDivisionUtil.getParams(arg)));

        try {
            batchDeleteService.batchDeleteRent(arg, session);
        } catch (Exception e) {
            LOGGER.error("error message = {}", e.getMessage());
            return new Result(false, -1, e.getMessage());
        }
